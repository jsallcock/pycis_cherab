...
def prepare_cis_instrument_components():
    """ Create the instances of the CIS instrument components required for the modelling."""

    # generate photron SA4 camera
    ...
    crystal_wp = pycis.model.Waveplate('ccfe_4.48mm_waveplate', 4.48e-3)
    crystal_wp.save()

    # generate savart plate

    crystal_sp = pycis.model.Savartplate('ccfe_6.2mm_savartplate', 6.2e-3)
    crystal_sp.save()

    # generate instrument

    inst = pycis.model.Instrument('mastu_ciii_cherab', instrument_contrast=0.7)

    inst.add_camera('photron_SA4')
    inst.add_lens_3('sigma_150mm')
    # inst.add_filter('ccfe_CII_m')

    inst.add_crystal('ccfe_4.48mm_waveplate', orientation=3 * np.pi / 4)
